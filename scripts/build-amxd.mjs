...
const wrapperJs = readFileSync(path.join(root, "chordprog-wrapper.js"), "utf8");
...
// Max's virtual filesystem and never exist on disk, so jweb (Chromium) can't
// load them - but wrapper.js itself always runs, and it extracts the payload
// to a real file next to the .amxd on first load.
let uiHtml = null;
...
function uiPayloadJs(buf) {
	...
	return (
		"\n// ---- generated by build-amxd.mjs: embedded UI payload ----\n" +
		`var UI_PAYLOAD_BYTES = ${buf.length};\n` +
		`var UI_PAYLOAD_B64 = [\n${chunks.join(",\n")}\n];\n`
	);
}

const wrapperData = uiHtml !== null ? wrapperJs + uiPayloadJs(uiHtml) : wrapperJs;
...
const files = [
	...
	{ type: "TEXT", name: "chordprog-wrapper.js", data: Buffer.from(wrapperData, "utf8"), flag: 0 },
];
