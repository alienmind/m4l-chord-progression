 ...
 * 1. Move dist/index.html → dist/<name>/chordprog-ui.html
...
await rename(path.join(dist, "index.html"), path.join(outDir, "chordprog-ui.html"));
console.log(`postbuild: dist/index.html → dist/${name}/chordprog-ui.html`);

execFileSync(process.execPath, [
	path.join(root, "scripts", "build-amxd.mjs"),
	path.join(outDir, "alienmind-chord-progression-m4l.amxd"),
], { stdio: "inherit" });

await copyFile(path.join(root, "chordprog-wrapper.js"), path.join(outDir, "chordprog-wrapper.js"));
console.log(`postbuild: chordprog-wrapper.js → dist/${name}/chordprog-wrapper.js`);
...
await new Promise((resolve, reject) => {
	...
	archive.pipe(output);
	for (const f of ["alienmind-chord-progression-m4l.amxd", "chordprog-wrapper.js", "chordprog-ui.html"]) {
